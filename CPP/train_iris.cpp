...
int main(){
    int num_class = 3;
    std::vector<int> hidden_dims= std::vector<int>({16,32,3});
    int input_dim = 4;
    ...
    auto train_ds=read_file("/mnt/01D4B61792FFD5D0/btl_co_chi/src/datasets/iris/train.txt");
    auto test_ds = read_file("/mnt/01D4B61792FFD5D0/btl_co_chi/src/datasets/iris/test.txt");
    ...
    double best_acc = 0;
    std::string checkpoint="./iris.ckpt";
    std::random_shuffle(indexs.begin(), indexs.end());
    for(int epoch=0; epoch<300; epoch++){
        cout<<"Training at epoch "<<to_string(epoch)<<endl;
        if(epoch == 100)optim.setLr(optim.getLr()/10.);
        if(epoch == 200)optim.setLr(optim.getLr()/10.);
        if(epoch == 250)optim.setLr(optim.getLr()/10.);
        double loss_total=0;
        int acc_total=0;
        std::random_shuffle(indexs.begin(), indexs.end()); 
